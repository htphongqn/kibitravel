...
namespace Controller
{
    public class Attfile
    {
        ...
        public List<Attfile_entity> Load_att2(string _news_seo)
        {
            List<Attfile_entity> l = new List<Attfile_entity>();
            var list = (from b in db.ESHOP_NEWS_ATTs
                        where b.ESHOP_NEW.NEWS_SEO_URL == _news_seo
                        select new
                        {
                            NEWS_ATT_FILE = b.NEWS_ATT_FILE,
        ...
        public string BindAttItems2(object News_Id, object Ext_Id, object Att_Name, object Att_Url, object Att_File)
        {
            try
            {
                string url = "";
                ...
                strResult += "<a class='btn-mn' href='" + url + "' target='_blank' title='" + Utils.CStrDef(Att_Name) + "'>" + "Download Now" + "</a>";
